...
float temperature, humidity, dht_temperature, heat_index;
...
void handle_http_metrics() {
	...
	"# HELP " PROM_NAMESPACE "_humidity_percent Air humidity.\n"
        "# TYPE " PROM_NAMESPACE "_humidity_percent gauge\n"
        "# UNIT " PROM_NAMESPACE "_humidity_percent %%\n"
        PROM_NAMESPACE "_humidity_percent %f\n"
        "# HELP " PROM_NAMESPACE "_dht_temperature_fahrenheit Air temperature.\n"
        "# TYPE " PROM_NAMESPACE "_dht_temperature_fahrenheit gauge\n"
        "# UNIT " PROM_NAMESPACE "_dht_temperature_fahrenheit \u00B0F\n"
        PROM_NAMESPACE "_dht_temperature_fahrenheit %f\n"
        "# HELP " PROM_NAMESPACE "_heat_index_fahrenheit Apparent air temperature, based on temperature and humidity.\n"
        "# TYPE " PROM_NAMESPACE "_heat_index_fahrenheit gauge\n"
        "# UNIT " PROM_NAMESPACE "_heat_index_fahrenheit \u00B0F\n"
        PROM_NAMESPACE "_heat_index_fahrenheit %f\n";

    read_sensors();        
    if (isnan(temperature) || isnan(humidity) || isnan(dht_temperature) || isnan(heat_index)) {    
        http_server.send(500, "text/plain; charset=utf-8", "Sensor error.");
        return;
    }

    char response[BUFSIZE];    
    snprintf(response, BUFSIZE, response_template, VERSION, BOARD_NAME, TEMPERATURE_SENSOR_NAME, temperature, humidity, dht_temperature, heat_index);
    http_server.send(200, "text/plain; charset=utf-8", response);
}
...
void read_dht_temperature_sensor() {
    log("Reading DHT temperature sensor ...", LogLevel::DEBUG);
    bool result = read_sensor([] {
        return dht_sensor.getTemperature();
    }, &dht_temperature);
    if (result) {
        dht_temperature += TEMPERATURE_CORRECTION_OFFSET;
        dht_temperature = dht_sensor.toFahrenheit(dht_temperature);
    } else {
...
void read_heat_index() {
    if (!isnan(humidity) && !isnan(dht_temperature)) {
        heat_index = dht_sensor.computeHeatIndex(dht_temperature, humidity, true);
    } else {
        heat_index = NAN;
    }
}
